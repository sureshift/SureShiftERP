...
migrate((db) => {
  ...
  // ── Helper to save quietly if collection already exists ──────────────
  function saveCol(col) {
    try { return db.saveCollection(col); }
    catch(e) { if (!String(e).includes("already exists")) throw e; }
  }

  // ── 1. USERS (auth collection) ────────────────────────────────────────
  saveCol(new Collection({
    name: "users", type: "auth",
    schema: [
      { name:"name",        type:"text",   required:true  },
      { name:"phone",       type:"text",   required:false },
      { name:"role",        type:"select", required:true,
        options:{ maxSelect:1, values:["super_admin","branch_head","sales_exec","ops_exec","finance_exec","surveyor","vehicle_vendor","manpower_vendor"] }},
      { name:"branch",      type:"select", required:true,
        options:{ maxSelect:1, values:["NDLH","MUMB","BANG","CHEN","HYDB","KOLK"] }},
      { name:"status",      type:"select", required:true,
        options:{ maxSelect:1, values:["active","inactive","suspended"] }},
  ...
  saveCol(new Collection({
    ...
    schema:[
      ...
      { name:"type",           type:"select", required:true,
        options:{ maxSelect:1, values:["vehicle_vendor","manpower_vendor"] }},
  ...
  saveCol(new Collection({
    ...
    schema:[
      ...
      { name:"move_type",    type:"select", required:true,
        options:{ maxSelect:1, values:["household","office","international","vehicle","bike","storage","commercial","courier"] }},
      { name:"source",       type:"select", required:true,
        options:{ maxSelect:1, values:["website","gmb","phone","whatsapp","reference"] }},
      { name:"stage",        type:"select", required:true,
        options:{ maxSelect:1, values:["new","survey","quotation","recalling","cfr","lost"] }},
  ...
  saveCol(new Collection({
    ...
    schema:[
      ...
      { name:"status",         type:"select", required:true,
        options:{ maxSelect:1, values:["pending","assigned","scheduled","in-progress","completed","report-filed"] }},
  ...
  saveCol(new Collection({
    ...
    schema:[
      ...
      { name:"enquiry_id",   type:"text",   required:true  },
      { name:"status",       type:"select", required:true,
        options:{ maxSelect:1, values:["draft","sent","viewed","negotiating","approved","recalling","converted","lost"] }},
  ...
  saveCol(new Collection({
    ...
    schema:[
      ...
      { name:"enquiry_id",     type:"text",   required:true  },
      { name:"status",         type:"select", required:true,
        options:{ maxSelect:1, values:["token-pending","token-received","confirmed","vendor-assigned","ops-ready","in-transit","delivered","cancelled"] }},
  ...
  saveCol(new Collection({
    ...
    schema:[
      ...
      { name:"stage",            type:"select", required:true,
        options:{ maxSelect:1, values:["dispatch-mat","packing","loading","in-transit","unloading","delivered"] }},
  ...
  saveCol(new Collection({
    ...
    schema:[
      ...
      { name:"cfr_id",          type:"text",   required:true  },
      { name:"status",          type:"select", required:true,
        options:{ maxSelect:1, values:["draft","sent","partial","paid","overdue","cancelled"] }},
      { name:"line_items",      type:"json",   required:false },
      { name:"subtotal",        type:"number", required:false },
      { name:"tax_amt",         type:"number", required:false },
      { name:"grand_total",     type:"number", required:true  },
  ...
  saveCol(new Collection({
    ...
    schema:[
      ...
      { name:"category",  type:"select", required:true,
        options:{ maxSelect:1, values:["technical","billing","operations","user-access","feature-request","other"] }},
      { name:"priority",  type:"select", required:true,
        options:{ maxSelect:1, values:["low","medium","high","critical"] }},
      { name:"status",    type:"select", required:true,
        options:{ maxSelect:1, values:["open","in-progress","resolved","closed"] }},
  ...
  saveCol(new Collection({
    ...
    schema:[
      { name:"entity_type", type:"select", required:true,
        options:{ maxSelect:1, values:["enquiry","quotation","cfr","operation"] }},
      { name:"entity_id",   type:"text",   required:true  },
      { name:"channel",     type:"select", required:true,
        options:{ maxSelect:1, values:["whatsapp","email","sms","call"] }},
      { name:"direction",   type:"select", required:true,
        options:{ maxSelect:1, values:["sent","received","auto"] }},
      { name:"message",     type:"text",   required:true  },
      { name:"read_receipt",type:"bool",   required:false },
